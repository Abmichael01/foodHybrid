...
const formSchema = z.object({
  ...
  transaction_pin: z.string().length(4, "PIN must be 4 digits").optional(),
});
...
const EnterPin: React.FC = () => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      if (hasPin) {
        updatePinData({ transaction_pin: form.getValues().transaction_pin, pin: form.getValues().pin });
        navigate("/partner/profile/withdrawal-pin/change-pin?current=confirmOtp");
      } else {
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    if (hasPin) {
      ...
      mutate({ transaction_pin: data.transaction_pin, pin: data.pin });
    }
  };
            ...
            render={({ field }) => (
              <FormItem className="space-y-[24px] font-satoshi flex flex-col items-center">
                <FormLabel className="font-[500] text-[16px]">
                  {hasPin ? "Enter current" : "Set"} Transaction PIN
                </FormLabel>
                <FormControl>
                  <InputOTP
                    maxLength={4}
